...
public class ChatController {
    ...
    public ResponseEntity<?> uploadImage(@ModelAttribute ImageDto dto)throws Exception{
        response = new ResponseService("failed");
        try {
            if(dto.getFile() == null){
                System.out.println("Empty");
            }
            fileUploader.upload(dto.getFile());
            response.setMessage("File uploaded");
        } catch (Exception e) {
            System.out.println("Error " +  e);
            response.setErrorMessage("Not done");
        }
